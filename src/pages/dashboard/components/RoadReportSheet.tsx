...
export function RoadReportSheet({ location, onClose, onCreated, onLayoutChange, onDesktopDragStart, onDesktopDragMove, onDesktopDragEnd, onDesktopDragKeyDown, mobileHandle }: RoadReportSheetProps) {
  ...
  return <div className="relative flex h-full min-h-0 flex-col">
    ...
    <header className="flex h-14 shrink-0 items-center gap-3 border-b border-ae-line px-5">
      {step === 2 && <button className="grid size-10 shrink-0 place-items-center rounded-sm text-ae-muted hover:bg-ae-soft" type="button" aria-label="Back to categories" onClick={() => setStep(1)}><ArrowLeft className="size-5" /></button>}<button className="hidden min-w-0 flex-1 touch-none cursor-move items-center gap-3 text-left text-ae-muted lg:flex" type="button" aria-label="Drag report panel" onPointerDown={onDesktopDragStart} onPointerMove={onDesktopDragMove} onPointerUp={onDesktopDragEnd} onPointerCancel={onDesktopDragEnd} onKeyDown={onDesktopDragKeyDown}><GripHorizontal className="size-5 shrink-0" aria-hidden="true" /><span className="truncate text-base font-black text-ae-ink">{step === 1 ? 'What is happening?' : 'Add useful details'}</span></button><h2 className="m-0 min-w-0 flex-1 truncate text-[15px] font-black lg:hidden" id="report-title">{step === 1 ? 'What is happening?' : 'Add useful details'}</h2>
