...
const modes = [
  ...
  { value: 'BICYCLE' as const, label: 'Cycle', Icon: Bike },
  { value: 'BUS' as const, label: 'Bus', Icon: BusFront },
  { value: 'TRAIN' as const, label: 'Train', Icon: TrainFront },
  { value: 'SUBWAY' as const, label: 'Subway', Icon: TrainFrontTunnel },
]
...
export function PlannerPanel(props: PlannerPanelProps) {
  ...
  return <>
    ...
    <div className="flex h-12 shrink-0 items-center gap-3 border-b border-ae-line px-5 lg:h-14"><button className="hidden min-w-0 flex-1 touch-none cursor-move items-center gap-3 text-left text-ae-muted lg:flex" type="button" aria-label="Drag planner panel" onPointerDown={props.onDesktopDragStart} onPointerMove={props.onDesktopDragMove} onPointerUp={props.onDesktopDragEnd} onPointerCancel={props.onDesktopDragEnd} onKeyDown={props.onDesktopDragKeyDown}><GripHorizontal className="size-5 shrink-0" aria-hidden="true" /><span className="truncate whitespace-nowrap text-base font-black text-ae-ink">Where are you going?</span></button><h2 className="m-0 flex-1 truncate whitespace-nowrap text-[15px] font-black lg:hidden">Where are you going?</h2><button className="grid size-11 shrink-0 place-items-center rounded-lg text-ae-muted hover:bg-ae-soft hover:text-ae-brand" type="button" aria-label="Close planner panel" onClick={props.onClose}><X className="size-5" aria-hidden="true" /></button></div>
    <div className="min-h-0 flex-1 touch-pan-y overflow-y-auto overscroll-y-contain px-5 py-3 [scrollbar-width:thin] [scrollbar-color:#b7c8c0_transparent] [-webkit-overflow-scrolling:touch] sm:px-6 lg:h-auto lg:px-7 lg:pt-5 lg:pb-6">
      <form id="route-planner-form" className="grid gap-3 lg:gap-4" onSubmit={props.onSubmit} noValidate>
        ...
        <fieldset className="min-w-0 border-0 p-0"><legend className="mb-1 text-[13px] font-extrabold lg:text-sm">Travel mode</legend><p className="mt-0 mb-2 text-[11px] font-semibold text-ae-muted lg:text-xs">Choose up to 3 modes. Available combinations will be shown.</p><div className="grid min-w-0 grid-cols-5 gap-1.5">{modes.map(({ value, label, Icon }) => { const checked = props.selectedModes.includes(value); const disabled = !checked && props.selectedModes.length === 3; return <label className={`flex min-h-13 min-w-0 flex-col items-center justify-center gap-1 overflow-hidden rounded-lg border px-1 text-[11px] font-black lg:text-xs ${checked ? 'cursor-pointer border-ae-brand bg-ae-soft text-ae-brand' : disabled ? 'cursor-not-allowed border-ae-line bg-white opacity-45' : 'cursor-pointer border-ae-line bg-white hover:border-ae-brand'}`} key={value}><input className="sr-only" type="checkbox" name="direct-mode" checked={checked} disabled={disabled} onChange={() => toggleMode(value)} /><Icon className="size-5 shrink-0" aria-hidden="true" /><span className="min-w-0 truncate">{label}</span></label> })}</div></fieldset>
        {hasActive && <fieldset className="border-0 p-0"><legend className="mb-2 text-[13px] font-extrabold lg:text-sm">Active route priority</legend><div className="grid grid-cols-2 gap-2">{[{ value: 'balanced' as const, label: 'Balanced', icon: colorBalancedPriorityIcon }, { value: 'lower-exposure' as const, label: 'Lower exposure', icon: colorLowerExposureIcon }].map((option) => <label className={`flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border px-3 text-xs font-black lg:text-sm ${props.preference === option.value ? 'border-ae-brand bg-ae-soft text-ae-brand' : 'border-ae-line'}`} key={option.value}><input className="sr-only" type="radio" name="active-route-priority" checked={props.preference === option.value} onChange={() => props.onPreferenceChange(option.value)} /><img className="size-6 object-contain lg:size-7" src={option.icon} alt="" aria-hidden="true" />{option.label}</label>)}</div></fieldset>}
        {hasTransit && <fieldset className="border-0 p-0"><legend className="mb-2 text-[13px] font-extrabold lg:text-sm">Transit priority</legend><div className="grid grid-cols-2 gap-2">{[{ value: 'LESS_WALKING' as const, label: 'Less walking', icon: colorWalkingIcon }, { value: 'FEWER_TRANSFERS' as const, label: 'Fewer transfers', icon: colorCompareIcon }].map((option) => <label className={`flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3 text-xs font-black lg:text-sm ${props.transitPreference === option.value ? 'border-ae-brand bg-ae-soft text-ae-brand' : 'border-ae-line'}`} key={option.value}><input className="sr-only" type="radio" name="transit-priority" checked={props.transitPreference === option.value} onChange={() => props.onTransitPreferenceChange(option.value)} /><img className="size-6 object-contain lg:size-7" src={option.icon} alt="" aria-hidden="true" />{option.label}</label>)}</div></fieldset>}
        <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-lg border border-ae-line p-3"><input className="mt-1 size-4 accent-ae-brand" type="checkbox" checked={props.sensitiveUser} onChange={(event) => props.onSensitiveUserChange(event.target.checked)} /><span><strong className="block text-[13px] font-black lg:text-sm">Sensitive-user mode</strong><small className="mt-1 block text-[11px] font-semibold text-ae-muted lg:text-xs">Balanced considers lower-exposure routes up to 35% slower instead of 20%.</small></span></label>
        <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-lg border border-ae-line p-3"><input className="mt-1 size-4 accent-ae-brand" type="checkbox" aria-label="Reduced exertion" checked={props.accessibilityMode === 'REDUCED_EXERTION'} onChange={(event) => props.onAccessibilityModeChange(event.target.checked ? 'REDUCED_EXERTION' : 'STANDARD')} /><span><strong className="block text-[13px] font-black lg:text-sm">Reduced exertion</strong><small className="mt-1 block text-[11px] font-semibold text-ae-muted lg:text-xs">Approximation only; it does not verify wheelchair or step-free access.</small></span></label>
        <button className="hidden min-h-13 items-center justify-center gap-2 rounded-lg bg-ae-ink px-5 text-sm font-black text-white hover:bg-ae-brand disabled:opacity-60 lg:inline-flex" type="submit" disabled={props.isPending}>{props.isPending ? 'Comparing...' : 'Compare routes'}<ArrowRight className="size-5" aria-hidden="true" /></button>
      </form>
    </div>
