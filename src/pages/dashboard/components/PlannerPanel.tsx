  ...
  return <>
    ...
    <div className="h-0 flex-1 touch-pan-y overflow-y-scroll overscroll-y-contain px-5 pt-3 pb-28 [scrollbar-width:thin] [scrollbar-color:#b7c8c0_transparent] [-webkit-overflow-scrolling:touch] sm:px-6 lg:h-auto lg:px-7 lg:pt-5 lg:pb-6 [@media(min-width:1024px)_and_(max-height:900px)]:px-5 [@media(min-width:1024px)_and_(max-height:900px)]:pt-3 [@media(min-width:1024px)_and_(max-height:900px)]:pb-4">
      <form id="route-planner-form" className="grid gap-4 [@media(min-width:1024px)_and_(max-height:900px)]:gap-3 [@media(min-width:1024px)_and_(max-height:760px)]:gap-2" onSubmit={props.onSubmit} noValidate>
        ...
        <div className="flex items-center justify-between gap-3"><button className="inline-flex min-h-10 items-center gap-2 rounded-lg px-2 text-sm [@media(min-width:1024px)_and_(max-height:760px)]:text-xs font-extrabold text-ae-brand hover:bg-ae-soft" type="button" disabled={props.isLocating} onClick={props.onCurrentLocation}><LocateFixed className="size-4" aria-hidden="true" />{props.isLocating ? 'Finding location...' : 'Use current location'}</button><button className="inline-flex min-h-10 items-center gap-2 rounded-lg px-3 text-sm font-extrabold text-ae-muted hover:bg-ae-soft hover:text-ae-brand" type="button" onClick={props.onSwap}><ArrowUpDown className="size-4" aria-hidden="true" />Swap</button></div>
    ...
    <div className="absolute inset-x-0 bottom-0 z-20 border-t border-ae-line bg-white/96 px-5 pt-3 pb-[max(.75rem,env(safe-area-inset-bottom))] backdrop-blur-xl lg:hidden"><button className="inline-flex min-h-13 w-full items-center justify-center gap-2 rounded-lg bg-ae-ink px-5 font-black text-white hover:bg-ae-brand disabled:opacity-60" type="submit" form="route-planner-form" disabled={props.isPending}>{props.isPending ? 'Comparing...' : 'Compare routes'}<ArrowRight className="size-5" aria-hidden="true" /></button></div>
  </>
}
