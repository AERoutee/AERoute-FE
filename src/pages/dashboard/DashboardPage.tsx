...
export default function DashboardPage() {
  ...
  const [mapLayers, setMapLayers] = useState({ airQuality: false, weather: false, reports: true })
  const [layerAvailability, setLayerAvailability] = useState({ airQuality: true })
  ...
  const hasWeatherPoints = Boolean(selectedWeatherPoints?.some((point) => point.conditions.status === 'available'))
  ...
  return <main id="main-content" className="relative h-full overflow-hidden bg-white text-ae-ink" tabIndex={-1}>
    <section className="absolute inset-0" aria-label="Route map"><RoutePreviewMap origin={origin} destination={destination} routes={routes} selectedId={selected?.id} liveLocation={liveLocation} followLiveLocation={isNavigating} reports={reports} navigationRoute={isNavigating ? selected : null} showAirQuality={mapLayers.airQuality} showWeather={mapLayers.weather} weatherPoints={selectedWeatherPoints} showReports={mapLayers.reports} onAirQualityAvailabilityChange={(available) => { setLayerAvailability((current) => ({ ...current, airQuality: available })); if (!available) setMapLayers((current) => ({ ...current, airQuality: false })) }} onNavigationProgress={(progress) => setNavigationProgress({ remainingMeters: progress.remainingMeters, isOffRoute: progress.isOffRoute })} onOriginChange={handleOriginCheckpointChange} onDestinationChange={handleDestinationCheckpointChange} onBoundsChange={handleMapBoundsChange} onRouteSelect={setSelectedId} /></section>
