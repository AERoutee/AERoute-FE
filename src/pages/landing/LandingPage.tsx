...
const steps = [
  { number: '01', icon: colorMapMarkerIcon, title: 'Tentukan perjalanan Anda', detail: 'Cari alamat, stasiun, kampus, atau tempat penting.' },
  { number: '02', icon: colorBalanceIcon, title: 'Lihat pertimbangannya', detail: 'Bandingkan waktu, jarak, dan perkiraan paparan secara berdampingan.' },
  { number: '03', icon: colorWindIcon, title: 'Temukan pilihan yang lebih baik', detail: 'Rute yang tiga menit lebih lama tetap bisa menjadi pilihan yang lebih cerdas.' },
  { number: '04', icon: colorBicycleIcon, title: 'Bergerak dengan informasi lengkap', detail: 'Pilih berjalan kaki atau bersepeda dan mulai perjalanan Anda dengan gambaran yang lebih jelas.' },
]
    ...
    <main ref={pageRef} id="main-content" className="relative isolate overflow-hidden bg-white text-ae-ink" tabIndex={-1}>
      ...
      <section className="relative z-10 flex min-h-[calc(100svh-72px)] items-center px-5 py-10 sm:px-8 lg:px-12" aria-labelledby="hero-title">
        <div className="relative z-10 mx-auto grid w-full max-w-[90rem] lg:grid-cols-[minmax(0,41rem)_1fr] lg:gap-20">
          <div className="self-center py-6 pr-[24%] sm:py-8 sm:pr-0">
            <h1 className="m-0 text-[clamp(2.75rem,12vw,6.2rem)] leading-[.88] font-black tracking-[-.07em]" id="hero-title">Tidak semua rute <span className="text-ae-brand">terpendek</span> adalah yang terbaik.</h1>
            <p className="mt-7 mb-0 max-w-[35rem] text-lg leading-8 font-semibold text-ae-muted sm:text-xl">AERoute membantu Anda membandingkan waktu dan perkiraan paparan PM2.5 sebelum memilih cara bepergian.</p>
            <div className="mt-9 flex flex-col gap-3 sm:flex-row"><a className="inline-flex min-h-14 items-center justify-center gap-2 rounded-xl bg-ae-ink px-7 py-4 text-base font-black text-white no-underline transition hover:bg-ae-brand" href="#how-it-works">Jelajahi ceritanya <ArrowRight className="size-5" aria-hidden="true" /></a>{session.isPending ? <span className="h-14 w-36 animate-pulse rounded-xl bg-ae-soft" aria-label="Memuat akun" /> : <Link className="inline-flex min-h-14 items-center justify-center gap-2 rounded-xl border border-ae-border bg-white px-7 py-4 text-base font-black text-ae-ink no-underline hover:border-ae-brand hover:text-ae-brand" to={isSignedIn ? '/dashboard' : '/login'}>{isSignedIn ? <LayoutDashboard className="size-5" aria-hidden="true" /> : <LogIn className="size-5" aria-hidden="true" />}{isSignedIn ? 'Buka dasbor' : 'Masuk'}</Link>}</div>
          </div>
      ...
      <section className="relative z-10 px-5 py-20 sm:px-8 lg:px-12 lg:py-28" id="how-it-works" aria-labelledby="story-title">
        <div className="mx-auto max-w-[90rem]">
          <div className="max-w-[31rem] py-4 pr-[24%] sm:pr-0"><p className="mb-3 text-xs font-black tracking-[.16em] text-ae-brand uppercase">Cerita perjalanan</p><h2 className="m-0 text-4xl leading-[.98] font-black tracking-[-.055em] sm:text-6xl" id="story-title">Satu perjalanan.<br /><span className="text-ae-brand">Empat keputusan yang lebih baik.</span></h2></div>
      ...
      <section className="relative z-10 px-5 py-20 sm:px-8 lg:px-12 lg:py-28" aria-labelledby="proof-title"><div className="mx-auto grid max-w-[90rem] items-center gap-12 lg:grid-cols-[.7fr_1.3fr]"><div className="py-4 pr-[24%] sm:pr-0"><p className="mb-3 text-xs font-black tracking-[.16em] text-ae-brand uppercase">Perbedaan yang berarti</p><h2 className="m-0 text-4xl leading-[.98] font-black tracking-[-.055em] sm:text-6xl" id="proof-title">Cepat hanyalah satu bagian dari perjalanan.</h2><p className="mt-5 mb-0 max-w-md text-lg leading-8 font-semibold text-ae-muted">AERoute menampilkan pertimbangan tersembunyi sebelum Anda berangkat.</p></div><div className="mr-[24%] grid gap-4 sm:mr-0 sm:grid-cols-3"><div className="rounded-xl bg-ae-ink p-6 text-white"><strong className="text-5xl font-black">+3</strong><span className="mt-2 block text-sm font-bold text-[#c5e9d8]">menit pada rute rekomendasi</span></div><div className="rounded-xl bg-ae-soft p-6"><strong className="text-5xl font-black text-ae-brand">41%</strong><span className="mt-2 block text-sm font-bold text-ae-muted">perkiraan paparan lebih rendah</span></div><div className="rounded-xl bg-white p-6 shadow-[0_18px_45px_rgba(20,41,34,.1)]"><strong className="text-5xl font-black">2</strong><span className="mt-2 block text-sm font-bold text-ae-muted">cara bepergian: berjalan kaki atau bersepeda</span></div></div></div></section>

      <footer className="relative z-10 px-5 pb-20 sm:px-8 lg:px-12 lg:pb-28"><div className="mx-auto flex max-w-[90rem] flex-col items-start justify-between gap-8 rounded-xl bg-ae-ink p-8 text-white sm:p-12 lg:flex-row lg:items-center"><h2 className="m-0 max-w-2xl text-4xl leading-[.98] font-black tracking-[-.05em] sm:text-5xl">Jadikan setiap keputusan rute lebih jelas.</h2><Link className="inline-flex min-h-14 shrink-0 items-center justify-center gap-2 rounded-xl bg-white px-7 py-4 text-base font-black text-ae-ink no-underline hover:bg-ae-soft" to={isSignedIn ? '/dashboard' : '/register'}>{isSignedIn ? 'Buka dasbor' : 'Buat akun'} <ArrowRight className="size-5" aria-hidden="true" /></Link></div></footer>
    </main>
  )
}
