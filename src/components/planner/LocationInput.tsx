  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      <div className={`flex min-h-14 items-center gap-3 rounded-xl border bg-white px-4 transition [@media(min-width:1024px)_and_(max-height:900px)]:min-h-12 [@media(min-width:1024px)_and_(max-height:900px)]:gap-2 ${error ? 'border-ae-fastest' : 'border-ae-line focus-within:border-ae-brand focus-within:ring-4 focus-within:ring-ae-brand/10'}`}>
        ...
        <input className="min-w-0 flex-1 border-0 bg-transparent py-3 text-base font-bold text-ae-ink outline-none placeholder:text-ae-muted/75 [@media(min-width:1024px)_and_(max-height:900px)]:py-2 [@media(min-width:1024px)_and_(max-height:760px)]:text-sm" id={id} role="combobox" aria-autocomplete="list" aria-controls={listId} aria-expanded={open && suggestions.length > 0} aria-activedescendant={activeIndex >= 0 ? `${listId}-${activeIndex}` : undefined} aria-invalid={Boolean(error)} aria-describedby={describedBy} value={query} onChange={(event) => updateQuery(event.target.value)} onKeyDown={handleKeyDown} onFocus={() => setOpen(true)} placeholder="Cari alamat atau tempat" autoComplete="off" />
      ...
      {(status === 'unavailable' || status === 'error') && <p className="mt-2 text-xs font-semibold text-ae-fastest lg:text-sm" id={statusId} role="status">Pencarian lokasi tidak tersedia.</p>}
      ...
      {open && status === 'ready' && query.trim().length >= 2 && suggestions.length === 0 && <p className="absolute z-40 mt-2 w-full rounded-xl border border-ae-line bg-white p-4 text-sm font-semibold text-ae-muted shadow-lg">Tempat yang sesuai tidak ditemukan.</p>}
    </div>
  )
}
