...
describe('navigation reroute decisions', () => {
  it('requires a fresh nearby accurate raw fix to start and confirm arrival', () => {
    ...
    expect(routeGuidanceEligibility(place, currentLocation, { ...place, accuracy: 80, timestamp: now - 300_000 }, now)).toEqual({ eligible: true, code: 'ELIGIBLE', message: 'Lokasi saat ini siap di titik awal rute.' })
    ...
    expect(routeGuidanceEligibility(place, currentLocation, { ...place, accuracy: 80, timestamp: now - 600_001 }, now)).toEqual({ eligible: false, code: 'STALE_FIX', message: 'Lokasi sudah kedaluwarsa. Tunggu pembaruan lokasi.' })
    expect(routeGuidanceEligibility(place, currentLocation, { ...place, accuracy: 100, timestamp: now - 600_000 }, now)).toEqual({ eligible: true, code: 'ELIGIBLE', message: 'Lokasi saat ini siap di titik awal rute.' })
    expect(canStartNavigationFrom(place, currentLocation, { ...place, accuracy: 100, timestamp: now - 600_000 }, now)).toBe(true)
