...
describe('dashboard drag layout', () => {
  ...
  it('keeps only report creation draggable and renders details through the map popup', () => {
    ...
    expect(source).toContain('absolute right-3 left-3 z-50')
    expect(source).toContain("style={{ top: '9rem' }}")
    const layout = readFileSync(join(process.cwd(), 'src/components/layout/DashboardLayout.tsx'), 'utf8')
    expect(layout).toContain('fixed inset-x-3 top-[max(.75rem,env(safe-area-inset-top))] z-[100]')
  })
})
